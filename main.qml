...
ApplicationWindow {
    ...
    property string url: 'https://chat.whatsapp.com/IBHvkroLSzi0xYR6ezj4XG'
    //property string url: 'https://web.whatsapp.com/'
        ...
        Rectangle{
            ...
            Column{
                ...
                Boton{
                    ...
                    onClicking: {
                        if(!btnUpdate.up){
                            ...
                            var downloaded = unik.downloadGit('https://github.com/nextsigner/wwas', fd)
                ...
                Boton{
                    ...
                    r:app.fs*0.2
                    onClicking:{
                        var d="-git=https://github.com/nextsigner/unik-tools.git"
                        unik.setUnikStartSettings(d)
                        unik.restartApp()
                    }
                }
                Boton{
    ...
    BotonUX{
        //z:container.z+1000000
        text: 'Mostrar HTML'
        bg.color:'black'
        fontColor: 'white'
        onClicked: getElements()
    }
    ULogView{id:uLogView}
    UWarnings{id:uWarnings}
    function getElements(){
        //modWhatsapp.webEngineView.runJavaScript('document.body.innerHTML', function(result) {
        let numIndexUltimoAudio=0
        modWhatsapp.webEngineView.runJavaScript('document.getElementsByTagName(\'audio\').length', function(result) {
                //uLogView.showLog(unik.toHtmlEscaped(result.length))
                numIndexUltimoAudio=parseInt(result-1)
            //uLogView.showLog(result)
            modWhatsapp.webEngineView.runJavaScript('document.getElementsByTagName(\'audio\')['+numIndexUltimoAudio+'].play()', function(result2) {
                    //uLogView.showLog(unik.toHtmlEscaped(result.length))
                    //numIndexUltimoAudio=result
                //uLogView.showLog(result2)
            });
        });
    }
